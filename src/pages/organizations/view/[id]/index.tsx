...
import { deleteSongById } from 'apiSdk/songs';
...
function OrganizationViewPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<OrganizationInterface>(
    ...
    () =>
      getOrganizationById(id, {
        relations: ['user', 'event', 'meeting', 'song'],
      }),
  );
  ...
  const songHandleDelete = async (id: string) => {
    setDeleteError(null);
    try {
              ...
              {hasAccess('event', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Stack spacing={2} mb={8}>
                  <Flex justifyContent="space-between">
                    ...
                    <NextLink passHref href={`/events/create?organization_id=${data?.id}`}>
              ...
              {hasAccess('meeting', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Stack spacing={2} mb={8}>
                  <Flex justifyContent="space-between">
                    ...
                    <NextLink passHref href={`/meetings/create?organization_id=${data?.id}`}>
                      <Button colorScheme="blue" mr="4" as="a">
                        Create
                      </Button>
                    </NextLink>
              ...
              {hasAccess('song', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                <Stack spacing={2} mb={8}>
                  <Flex justifyContent="space-between">
                    ...
                    <NextLink passHref href={`/songs/create?organization_id=${data?.id}`}>
                      <Button colorScheme="blue" mr="4" as="a">
                        Create
                      </Button>
                    </NextLink>
                  ...
                  <TableContainer mb={4}>
                    <Table variant="simple">
                      ...
                      <Tbody>
                        {data?.song?.map((record) => (
                          <Tr cursor="pointer" onClick={() => router.push(`/songs/view/${record.id}`)} key={record.id}>
                            <Td>{record.title}</Td>
                            <Td>{record.artist}</Td>
                            <Td>
